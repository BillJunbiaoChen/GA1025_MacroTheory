...
\begin{align*}
    \mathcal{L}^1 & = \log c^t_t + \log c^t_{t+1} + \lambda ( y - c^t_t - \frac{p_{t+1} c^t_{t+1}}{p_t c^t_t}) \\ 
    \mathcal{L}^2 & = \log c^t_t + \log c^t_{t+1} + \lambda ( \frac{p_{t+1} Y}{p_t c^t_t} - c^t_t - \frac{p_{t+1} c^t_{t+1}}{p_t c^t_t})
\end{align*}
The FOCs for Type 1 Youth are 
\begin{align*}
    [c^t_t] \quad & \frac{1}{c^t_t} = \lambda \\ 
    [c^t_{t+1}] \quad & \frac{1}{c^t_{t+1}} = \lambda \frac{p_{t+1}}{p_t c^t_t}
\end{align*}
...
Similarly, we have 
\[
{c_t^t}^{, \text{type 2}} = \frac{Y p^t_{t+1}}{2 p^t_t}
\]
...
\]
\[
{s_t^t}^{, \text{type 2}} = 0 - \frac{Y p^t_{t+1}}{2 p^t_t} = - \frac{Y p^t_{t+1}}{2 p^t_t}
\]

\textbf{Since there is no trade between generations}, the consumption loan market clears within generations.
Thus, we have 
\[
\frac{y}{2} N_1 - \frac{Y p^t_{t+1}}{2 p^t_t} N_2 = 0 \Rightarrow y N_1 - \frac{Y N_2}{1 + r_t} = 0,
\]
