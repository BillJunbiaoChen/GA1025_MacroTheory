...
$F_{\max}(x_m) = \text{Pr}(x < x_m) \cdot \text{Pr}(x < x_m) = [F(x_m)]^2$.  

\begin{equation}
    v(w_m) = \max_{a \in \{\text{Accept, Reject} \}}  
    \bigg( \frac{w_m}{1 - \beta}, c + \beta \mathbb{E}_{\max w}[v(w_m')] \bigg)
\end{equation}
where $\mathbb{E}_{\max w}[v(w_m')] = \int_0^{B} v(w_m') \, d[F(w_m')]^2$.  
Since $\mathbb{E}_{\max w}[v(w_m')] = 2 \int_0^{B} v(w_m') f(w_m') F(w_m') dw' > \int_0^{B} v(w_m') f(w_m') dw'$,%
\footnote{
...
we have  
\[
\bar{w}_{\max} = (1 - \beta) \big(c + \beta \mathbb{E}_{\max w}[v(w_m')]\big)  
...
\begin{equation}
    v(w) = \max_{a \in \{\text{Accept, Reject} \} }  \bigg( \frac{w}{1 - \beta}, c + \beta \mathbb{E}_{\max w} [v(w')] \bigg)
\end{equation}
where $\mathbb{E}_{\max w} [v(w')] = n \int_{0}^B v(w') F^{n-1}(w')f(w') dw'$.
